...
- Normalize exported slug values to their numeric Unity Asset Store id, both in template-based exports and in the default CSV export path.

### ✅ Tests

- Added non-regression coverage for direct `%Slug%` rendering and raw CSV export normalization.
...
- **AFF14**: Ajout d'un lien `⚙️ Setup` dans la navbar et d'un bouton `⚙️ Setup` à droite du titre principal pour faciliter l'accès à la configuration de la source de données.
...
- AFF11/AFF12/AFF13 : Improve UI consistency and functionality in asset management interface
...
- Added regression coverage to ensure batch export always excludes invalid assets.

## [1.2.20] - 2026-04-22

### 🐛 Fixed

- `POST /api/export` now applies the active `filter_stack` from the UI so exported assets match the filtered view.

### ✅ Tests

- Added a regression test to confirm the interactive export endpoint no longer exports unfiltered rows.
