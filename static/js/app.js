...
* ============================================================================
* Description: Client-side logic (Vanilla JS) for the DataTables interface.
* Version: 1.2.9
* ============================================================================
*/
...
function initializeTable() {
  ...
  currentColumns = $('#assetsTable thead th')
    .map(function () {
      return $(this).text();
    })
    .get();
...
function analyzeColumnForBooleans(column, callback) {
  ...
  $.ajax({
    ...
    success: function (resp) {
      ...
      const allBool = Array.from(values).every((v) => ['true', 'false', '1', '0', 'yes', 'no', 'oui', 'non'].includes(v));
...
function loadColumnValues(column) {
  ...
  $.ajax({
    ...
    success: function (resp) {
      ...
      html += '</select>';
      html +=
        '<small class="text-muted d-block">Ctrl+Click pour multiselect. Le champ texte ci-dessus reste utilisable pour du texte libre.</small></div>';
